...
def get_data(filters=None):
	...
	# result = frappe.db.sql("""SELECT asst.serial_number, asst.item_name, item.brand, item.description, asst.purchase_date, asst.asset_name, asd.transaction_date , asst.location, tmi.target_location, asst.condition, asst.remark, tmi.target_location,
	# 		CASE WHEN YEAR(asd.transaction_date)=2015 THEN tmi.target_location ELSE 0 END AS '2015',
	# 		CASE WHEN YEAR(asd.transaction_date)=2022 THEN tmi.target_location ELSE 0 END AS '2022'
	# 		FROM tabAsset as asst JOIN tabItem as item ON asst.item_code = item.item_code 
	# 		LEFT JOIN `tabAsset Movement Item` as tmi ON asst.name = tmi.asset
	# 		LEFT JOIN `tabAsset Movement` as asd ON asd.name = tmi.parent
	# 	""" 
	# 	)
